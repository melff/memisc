...
Before we start, we adjust \code{knitr}'s output (with which this
...
names(nes1948)
```
Note that the variable names are all changed from uppercase to lowercase (SPSS 
does not distinguish uppercase and lowercase variable names and uppercase looks 
like shouting). Casefolding could have been suppressed by the call 
...
ftable(genTable(percent(vote3)~occup4,data=vote.48),
       row.vars=2)
```
...
```{r, ftable.digits=c(2,2,2,0)}
ftable(genTable(percent(vote3)~relig3,data=vote.48),
       row.vars=2)
```
...
```{r, ftable.digits=c(2,2,2,0)}
ftable(genTable(percent(vote3)~race2,data=vote.48),
       row.vars=2)
```
...
```{r, ftable.digits=c(2,2,2,0)}
ftable(genTable(percent(vote3)~total.income,data=vote.48),
       row.vars=2)
```
