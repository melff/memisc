...
contingency table. We use the `toLatex` method defined
for tables to get a nicely formatted output. (The generic
function `toLatex` is defined in package `utils`.)
...
showcasing the `panel.errbars` function of package `memisc`:
```{r,vote3-total-income}
...
```
`mtable` returns an object of class `"mtable"`. The
`print` method for this class produces output as seen above.
This output has a format close to the requirements of social science
publications. With the appropriate formatting methods for objects of this
class one can produce almost publication-ready output. For example,
`mtable_format_html` creates HTML output - as used by `show_html`, 
which can be imported into many word-processor software.
...
```{r,Termplot-mod35,fig.width=9,fig.height=9}
